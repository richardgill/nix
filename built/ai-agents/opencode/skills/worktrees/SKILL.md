...
~/Scripts/worktree-branch --no-switch --pull --binary oc --prompt-file "$PROMPT_FILE" my-feature
...
(cd ../main && ~/Scripts/worktree-branch --no-switch --pull --binary oc --prompt-file "$PROMPT_FILE" my-feature)

# New branch from ./main HEAD, no prompt file
~/Scripts/worktree-branch --no-switch --pull --binary oc my-feature

# Rare: new branch from remote main instead of local ./main HEAD
~/Scripts/worktree-branch --no-switch --pull --binary oc --prompt-file "$PROMPT_FILE" origin/main my-feature

# Existing remote branch, local branch/worktree becomes feature-branch
~/Scripts/worktree-branch --no-switch --pull --binary oc --prompt-file "$PROMPT_FILE" origin/feature-branch
```
