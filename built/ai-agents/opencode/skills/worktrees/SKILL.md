...
~/Scripts/worktree-branch --no-switch --pull --binary oc --prompt-file "$FILE" "$BRANCH"
...
$PROMPT
EOF
~/Scripts/worktree-branch --no-switch --pull --binary oc --prompt-file "$PROMPT_FILE" "$BRANCH"
```
...
~/Scripts/worktree-branch --no-switch --pull --binary oc --prompt-file docs/migration-spec.md migration
...
fix the login bug
EOF
~/Scripts/worktree-branch --no-switch --pull --binary oc --prompt-file "$PROMPT_FILE" fix-login
