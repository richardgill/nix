...
~/Scripts/worktree-branch --no-switch --pull --binary codex --prompt-file "$FILE" "$BRANCH"
...
$PROMPT
EOF
~/Scripts/worktree-branch --no-switch --pull --binary codex --prompt-file "$PROMPT_FILE" "$BRANCH"
```
...
~/Scripts/worktree-branch --no-switch --pull --binary codex --prompt-file docs/migration-spec.md migration
...
fix the login bug
EOF
~/Scripts/worktree-branch --no-switch --pull --binary codex --prompt-file "$PROMPT_FILE" fix-login
