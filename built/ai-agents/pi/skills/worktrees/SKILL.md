...
~/Scripts/worktree-branch --detached --pull --binary pi --prompt-file "$FILE" "$BRANCH"

# With a text prompt
~/Scripts/worktree-branch --detached --pull --binary pi --prompt "$PROMPT" "$BRANCH"
```
...
```bash
# From an issue plan
~/Scripts/worktree-branch --detached --pull --binary pi --prompt-file thoughts/shared/issues/10-add-auth/plan.md add-auth

# From a design doc
~/Scripts/worktree-branch --detached --pull --binary pi --prompt-file docs/migration-spec.md migration

# From a text prompt
~/Scripts/worktree-branch --detached --pull --binary pi --prompt "fix the login bug" fix-login

# From a remote branch
~/Scripts/worktree-branch --detached --pull --binary pi --prompt "implement the feature from the PR description" origin/feature-branch
```
