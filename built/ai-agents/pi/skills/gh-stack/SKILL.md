...
```bash
tmux-pi --task-slug '<task>' --prompt-file '<prompt-file>' --worktree '<bottom>' --model "openai-codex/gpt-6-astra" --thinking "high"
tmux-pi --task-slug '<task>' --prompt-file '<prompt-file>' --worktree '<child>' --source-ref '<parent>' --model "openai-codex/gpt-6-astra" --thinking "high"
```
