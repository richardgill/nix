...
- Treat these standalone messages as explicit authorization to run the corresponding command:
  ...
  - These commands run `git add .` if nothing is staged; run `git add <specific-files>` first if you want to commit only specific files.
  - `ci`: run `auto-commit "<commit message>"`
  - `cip`: run `auto-commit-push "<commit message>"`
  - `cipr`: run `auto-commit-pr "<commit message>"`
...
- Watch gh checks by running `gh pr checks --watch --fail-fast || gh run view --log-failed | tail -n 200` in background
  - “🟢”, “green”, “baby”, or “babysit” means: watch the gh checks until they're successful, make any small fixes, and rerun any transient failures until all checks are green. If the issue needs my input, notify me.
...
- In explanations, define any term or concept that is not well-known before using it.
- Prefer showing user-facing APIs over internal implementation details.
- Prefer usage examples that demonstrate how those APIs are called or applied.
...
- Show the control flow from the caller's perspective.
- Include relevant surrounding context, function signatures, and imports.
- Omit implementation details unless they are important to the proposed design.
- Keep it brief and focused on what is needed to review the proposal.
...
Run `overmux instance --json` to discover the running server's `instanceId` and `deepLinkPrefix`; do not assume the instance name matches the hostname.

Link to tmux using `<deepLinkPrefix>/tmux/<session>/<window>/<pane>`. Use numeric tmux IDs with the `$`, `@`, and `%` prefixes removed, not window or pane indices. For example, session `$1`, window `@2`, pane `%647` becomes `overmux://um790-4242/tmux/1/2/647`. Session-only (`overmux://um790-4242/tmux/1`) and session/window (`overmux://um790-4242/tmux/1/2`) links also work.
