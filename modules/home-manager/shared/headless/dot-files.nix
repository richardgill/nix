{
  ...
  scriptsEntries = lib.mapAttrs' (name: type:
    lib.nameValuePair "Scripts/${name}" {
      source = ../../dot-files/Scripts + "/${name}";
    }
  ) (lib.filterAttrs (name: type:
    type == "regular" && !(lib.hasSuffix ".mustache" name)
  ) scriptFiles);
in {
  home.file = scriptsEntries // {
    "Scripts/beep" = {
      text = builtins.readFile (utils.renderMustache "beep-script" ../../dot-files/Scripts/beep.mustache {
    ...
    ".zshenv" = {
      text = builtins.readFile (utils.renderMustache "zshenv" ../../dot-files/zshenv.mustache {
    ...
    ".zprofile".text = builtins.readFile (utils.renderMustache "zprofile" ../../dot-files/zprofile.mustache {
      isDarwin = pkgs.stdenv.isDarwin;
    });
    ".config/alacritty/alacritty.toml".text = builtins.readFile (utils.renderMustache "alacritty-config" ../../dot-files/alacritty/alacritty.toml.mustache {
      zshPath = "${pkgs.zsh}/bin/zsh";
    });
    ".config/alacritty/themes".source = ../../dot-files/alacritty/themes;
    ".config/ghostty/config".text = builtins.readFile (utils.renderMustache "ghostty-config" ../../dot-files/ghostty/config.mustache {
      isDarwin = pkgs.stdenv.isDarwin;
      isLinux = pkgs.stdenv.isLinux;
      zshPath = "${pkgs.zsh}/bin/zsh";
    });
    ".config/opencode/AGENTS.md".text = builtins.readFile (utils.renderMustache "opencode-agents" ../../dot-files/opencode/AGENTS.md.mustache {
      homeDir = homeDir;
    });
    ".config/nvim".source = ../../dot-files/nvim;
    ".config/oh-my-posh".source = ../../dot-files/oh-my-posh;
    ".config/ripgrep/config".text = builtins.readFile (utils.renderMustache "ripgreprc" ../../dot-files/ripgrep/ripgreprc.mustache {
      homeDir = homeDir;
    });
    ...
    ".zshrc" = {
      text = builtins.readFile (utils.renderMustache "zshrc" ../../dot-files/zsh/zshrc.mustache {
        isDarwin = pkgs.stdenv.isDarwin;
