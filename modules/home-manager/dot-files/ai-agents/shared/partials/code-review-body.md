...
- [ ] The default command to run to get all changes to review: {{#if (eq agent "claude")}}`~/Scripts/git-pr-diff`{{else}}`git diff main...HEAD`{{/if}} unless this is an obvious exception.
...
End with a todo list for the {{#if (eq agent "claude")}}calling agent{{else}}user{{/if}}:
